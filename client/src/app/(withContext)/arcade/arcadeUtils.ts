...
export async function runCodeOutputModeClientSide(
    ...
    try {
        ...
        const response = await lock.call(() => runCode(codeContent, programmingLanguage));
...
export async function submitCodeClientSide(
    ...
    try {
        ...
        if (RUN_CODE_RESPONSES[response.result.statusId] === RunCodeStatuses.ACCEPTED) {
            openPopupWith(
            ...
            openPopupWith(
                `Some test cases failed. Status: ${RUN_CODE_RESPONSES[response.result.statusId]}`,
                "Understood",
                null,
                () => {},
                () => {}
