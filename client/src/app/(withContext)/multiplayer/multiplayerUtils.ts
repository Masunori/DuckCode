 ...
 * @param activeTab The active tab in the code editor, only use this in multiplayer mode
 * @param setExecutionStatus Set the execution status of a specified user, only use this in multiplayer mode
 */
export async function runCodeOutputModeClientSide(
    ...
    try {
        ...
        if (setExecutionStatus && activeTab) {
            setExecutionStatus(activeTab, "codeSuccess");
        }
    } catch (err) {
        if (err instanceof LockUnavailableError) {
            ...
            if (setExecutionStatus && activeTab) {
                setExecutionStatus(activeTab, "codeError");
            }
            openPopupWith(
    ...
    try {
        ...
        for (let i = 0; i < response.results.length; i++) {
            if (RUN_CODE_RESPONSES[response.results[i].statusId] !== RunCodeStatuses.ACCEPTED) {
                ...
                if (setExecutionStatus && activeTab) {
                    setExecutionStatus(activeTab, "testCasesFailed");
                }
...
export type ExecutionStatus = "idle" | "running" | "codeError" | "codeSuccess" | "testCasesPassed" | "testCasesFailed"
...
export const EXECUTION_STATUS_INFORMATION: Record<ExecutionStatus, { abbr: string, desc: string, color: string }> = {
    ...
    "codeSuccess": { abbr: "S", desc: "Code ExecutionSuccess", color: "var(--success-code-text-border-color)" },
