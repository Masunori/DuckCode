    ...
    new GoogleStrategy(
        {
            ...
            callbackURL: "http://localhost:4000/auth/google/callback" // this will get the value be received
            ...
            try {
                ...
                const query =  `SELECT * FROM users.account WHERE provider_id = $1 AND email = $2`;
                const { rows } = await pool.query(query, [provider_id, email]);
                let user;
                if(rows.length === 0) {
